...
if (typeof firebase !== 'undefined') {
   ...
   // 🛡️ SECURITY FILTER: Menetralisir karakter bahaya (XSS & HTML Injection)
    function escapeHtml(text) {
        ...
        return text
            ...
            .replace(/</g, "&lt;")
            .replace(/>/g, "&gt;")
            .replace(/"/g, "&quot;")   /* Menetralisir tanda kutip dua */
            .replace(/'/g, "&#039;");  /* 🛑 INI DIA PENAWAR UNTUK KUTIP TUNGGAL (') */
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function renderMessagesFromFirebase() {
            ...
            messagesArray.forEach((item) => {
                ...
                // 🛡️ SECURITY FIX: Menggunakan data-attribute agar tanda petik tidak memecah JavaScript
                let deleteButtonHTML = `<button class="delete-btn" title="Hapus Pesan" data-id="${item.id}" data-name="${escapeHtml(rawName)}" onclick="window.triggerFirebaseDelete(this.getAttribute('data-id'), this.getAttribute('data-name'))">&times;</button>`;
